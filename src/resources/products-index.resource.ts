...
import { toProductIndexProjection, sortProductsForIndex } from '../models/projections';

const RESOURCE_URI = 'mcp://estacion-dulce/products#index';
const MAX_SIZE_BYTES = 512 * 1024; // 512KB limit

/**
 * Products index resource with caching
 * Compact projection: id, name, categoryId, price, stock
 */
export async function getProductsIndexResource(params?: {
  ifNoneMatch?: string;
  ...
  try {
    ...
    const result = await productService.listProducts(100); // Get more for index
    ...
    const projections = result.data.map(toProductIndexProjection);
    const sorted = sortProductsForIndex(projections);
    ...
    console.error(`getProductsIndexResource error:`, error);
    return {
      contents: [{
        type: 'text',
